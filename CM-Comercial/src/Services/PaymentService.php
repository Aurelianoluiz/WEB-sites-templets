...
final class PaymentService
{
    ...
    /**
     * Creates the order and reserves stock atomically. The remote gateway is
     * intentionally called after COMMIT so database row locks are never held
     * while waiting on a third-party HTTP service.
     *
     * @param array<int,array{product_id:int,quantity:int}> $items
     * @return array{order_id:int,payment_transaction_id:int,payment_status:string,provider_payment_id:?string,pix_qr_code:string,pix_qr_code_base64:string,pix_expires_at:?string}
     */
    public function createPixOrder(
        ...
        $created = $this->database->transaction(function () use ($pdo, $customerId, $items, $payerEmail, $idempotencyKey, $shippingAmount): array {
            $existing = $this->findOrderByIdempotency($pdo, $idempotencyKey);
            if ($existing !== null) {
                return $this->hydrateExistingPayment($pdo, $existing['id']);
            }
            ...
            foreach ($items as $item) {
                ...
                if ((int)$product['stock_quantity'] < $quantity) {
                    throw new RuntimeException('Insufficient stock for product ' . $product['sku'] . '.');
                }
            ...
            foreach ($productRows as [$product, $quantity]) {
                ...
                if ($stockStmt->rowCount() !== 1) {
                    throw new RuntimeException('Atomic stock reservation failed.');
                }
            }
        ...
        if (isset($created['payment_status'])) {
            return $created;
        }

        try {
            $gatewayResult = $this->gateway->createPixCharge(
                $created['order_id'],
                $created['amount'],
                $created['payer_email'],
                $created['payer_name'],
            ...
            $this->markPaymentFailure($created['payment_transaction_id'], $created['order_id']);
            throw $e;
        }
        ...
        $this->database->transaction(function ($pdo) use ($created, $gatewayResult): void {
            ...
            $stmt->execute([
                $gatewayResult['provider_payment_id'],
                $gatewayResult['status'],
                $gatewayResult['pix_qr_code'],
                $gatewayResult['pix_qr_code_base64'],
                $gatewayResult['pix_expires_at'],
                json_encode($gatewayResult['raw'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR),
                $created['payment_transaction_id'],
            ]);
            $orderStatus = $gatewayResult['status'] === 'paid' ? 'confirmed' : 'pending';
            $orderPaymentStatus = $gatewayResult['status'];
            $pdo->prepare('UPDATE orders SET status=?,payment_status=? WHERE id=?')->execute([$orderStatus, $orderPaymentStatus, $created['order_id']]);
        });

        return [
            'order_id' => $created['order_id'],
            'payment_transaction_id' => $created['payment_transaction_id'],
            ...
            'pix_qr_code_base64' => $gatewayResult['pix_qr_code_base64'],
            'pix_expires_at' => $gatewayResult['pix_expires_at'],
        ];
    }

    /** @return array{status:string,order_status:string,provider_payment_id:?string,pix_qr_code:string,pix_qr_code_base64:string,pix_expires_at:?string} */
    public function orderStatus(int $customerId, int $orderId): array
    {
        ...
        $stmt = $this->database->pdo()->prepare('SELECT pt.status, o.status AS order_status, pt.provider_payment_id, pt.pix_qr_code, pt.pix_qr_code_base64, pt.pix_expires_at FROM orders o JOIN payment_transactions pt ON pt.order_id=o.id WHERE o.id=? AND o.customer_id=? ORDER BY pt.id DESC LIMIT 1');
        ...
        return [
            ...
            'provider_payment_id' => $row['provider_payment_id'] !== null ? (string)$row['provider_payment_id'] : null,
            'pix_qr_code' => (string)($row['pix_qr_code'] ?? ''),
            'pix_qr_code_base64' => (string)($row['pix_qr_code_base64'] ?? ''),
            'pix_expires_at' => $row['pix_expires_at'] !== null ? (string)$row['pix_expires_at'] : null,
        ];
    }
    ...
    private function validateInputs(array $items, string $email, string $idempotencyKey, float $shippingAmount): void
    {
        ...
        foreach ($items as $item) {
            if ((int)($item['product_id'] ?? 0) < 1 || (int)($item['quantity'] ?? 0) < 1) {
    ...
    {
        $stmt = $pdo->prepare('SELECT id,status,provider_payment_id,pix_qr_code,pix_qr_code_base64,pix_expires_at FROM payment_transactions WHERE order_id=? ORDER BY id DESC LIMIT 1');
        $stmt->execute([$orderId]);
        $row = $stmt->fetch();
        if ($row === false) throw new RuntimeException('Existing order has no payment transaction.');
        return [
            'order_id' => $orderId,
            'payment_transaction_id' => (int)$row['id'],
    ...
    private function markPaymentFailure(int $paymentTransactionId, int $orderId): void
    {
        $this->database->transaction(function ($pdo) use ($paymentTransactionId, $orderId): void {
            $pdo->prepare('UPDATE payment_transactions SET status=\'failed\' WHERE id=? AND status=\'pending\'')->execute([$paymentTransactionId]);
