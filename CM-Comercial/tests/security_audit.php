...
$checks = [
    ...
    'strict_idempotency_repository' => str_contains($idempotency, 'INSERT INTO payment_audit_log') && str_contains($idempotency, '23000') && str_contains($idempotency, '1062'),
    ...
    'deadlock_suite_dual_connection' => str_contains($deadlockTest, '$a=db(); $b=db();'),
    ...
    'deadlock_suite_redelivery_idempotency' => str_contains($deadlockTest, 'redelivery') && str_contains($deadlockTest, 'afterSecond'),
...
if ($failed !== []) { echo 'FAILED_CHECKS: ' . implode(', ', $failed) . PHP_EOL; exit(1); }
echo "SECURITY_AUDIT_PASSED\n";
