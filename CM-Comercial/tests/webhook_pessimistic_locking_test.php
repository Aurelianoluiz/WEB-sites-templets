...
$repository=(string)file_get_contents($repositoryPath);$handler=(string)file_get_contents($handlerPath);
$checks=[
 'apply_webhook_transition_present'=>str_contains($repository,'applyWebhookTransition('),
 'payment_lock_requested'=>str_contains($repository,'lockPaymentForWebhook(')&&str_contains($repository,'lockOrderForWebhook('),
 'mysql_for_update_present'=>str_contains($repository,'FOR UPDATE'),
 'mysql_driver_guard'=>str_contains($repository,'ATTR_DRIVER_NAME')&&str_contains($repository,"'mysql'"),
...
if($failed!==[]){fwrite(STDERR,'FAILED: '.implode(', ',$failed).PHP_EOL);exit(1);}echo"PASS: webhook_pessimistic_locking_test\n";
